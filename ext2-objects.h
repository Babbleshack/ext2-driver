...
typedef struct __attribute__((packed)) ext2_superblock {
  ...
  uint32_t s_log_frag_size; //Fragment size (log)o
  ...
  uint16_t s_def_resuid; //Default UUID for reserved blocks
  uint16_t s_def_resgid; //Default GID for researved blocks
  ...
  uint32_t s_researved[204]; //pad to 1024
} ext2_superblock_t;
